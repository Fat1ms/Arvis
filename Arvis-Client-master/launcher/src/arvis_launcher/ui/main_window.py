...
from arvis_launcher.process import ManagedProcess
...
class LauncherMainWindow(QMainWindow):
    ...
    def _page_models(self) -> QWidget:
        ...
        hint = QLabel("Здесь будет список моделей (ollama list), pull/remove и выбор активной модели.")
    ...
    def _page_settings(self) -> QWidget:
        ...
        hint = QLabel(
            "Сейчас настройки portable: launcher_config.json. Позже: настройка клиента/моделей/путей и обновлений."
        )
        hint.setWordWrap(True)
        layout.addWidget(hint)
        layout.addStretch(1)
