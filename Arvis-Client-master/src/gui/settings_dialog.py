...
class SettingsDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self._nav_buttons = []
        # Note: "Общие", "Язык" and "Пользователи" tabs moved to Launcher
        sections = ["LLM", "TTS | STT", "Модули", "Расширенные"]
    ...
    def create_all_tabs(self):
        ...
        # Note: General, Language and Users tabs moved to Launcher
    ...
    def create_tts_stt_tab(self):
        ...
        # TTS Mode selection - скрыт, используется только "after_complete"
        # mode_layout = QHBoxLayout()
        # mode_layout.addWidget(QLabel("Режим озвучки:"))
        self.tts_mode_combo = QComboBox()
        self.tts_mode_combo.addItems(["После завершения"])  # Только один режим
        self.tts_mode_combo.setCurrentIndex(0)
        self.tts_mode_combo.setVisible(False)  # Скрываем комбобокс
        ...
        # Model path - скрыто, управляется из лаунчера
        # Создаём скрытые виджеты для совместимости с остальным кодом
        ...
        # Информация о модели (только чтение)
        model_info_layout = QHBoxLayout()
        model_info_label = QLabel("Модель STT:")
        self.stt_model_info = QLabel("Управляется через лаунчер")
        self.stt_model_info.setStyleSheet("color: #888; font-style: italic;")
        model_info_layout.addWidget(model_info_label)
        model_info_layout.addWidget(self.stt_model_info)
        model_info_layout.addStretch()
        stt_layout.addLayout(model_info_layout)
    ...
    def load_settings(self):
        ...
        # General tab widgets (moved to launcher, but may still exist in code)
        ...
        # Показываем имя текущей STT модели
        stt_model_path = str(self.config.get("stt.model_path", "") or "")
        if stt_model_path:
            from pathlib import Path
            model_name = Path(stt_model_path).name
            self.stt_model_info.setText(model_name)
        else:
            self.stt_model_info.setText("Не выбрана")
        ...
        # Load startup options (General tab moved to launcher)
        if hasattr(self, 'autostart_ollama'):
            self.autostart_ollama.setChecked(bool(self.config.get("startup.autostart_ollama", False)))
        if hasattr(self, 'preload_model'):
        ...
        # Load TTS mode settings - теперь всегда after_complete
        self.tts_mode_combo.setCurrentIndex(0)  # Только один режим
        ...
        # Load users tab settings (moved to launcher)
        if hasattr(self, 'auth_enabled_checkbox'):
            self.auth_enabled_checkbox.setChecked(bool(self.config.get("security.auth.enabled", False)))
        if hasattr(self, 'twofa_enabled_checkbox'):
        ...
        if hasattr(self, 'audit_enabled_checkbox'):
            self.audit_enabled_checkbox.setChecked(bool(self.config.get("audit.enabled", True)))

        # Update current user info if available (moved to launcher)
        if hasattr(self, 'current_user_label') and self.current_user_id:
            try:
        ...
        elif hasattr(self, 'current_user_label'):
            self.current_user_label.setText("Не аутентифицирован")
            if hasattr(self, 'change_password_button'):
                self.change_password_button.setEnabled(False)
            if hasattr(self, 'setup_2fa_button'):
                self.setup_2fa_button.setEnabled(False)
        # Language tab (moved to launcher)
    ...
    def apply_settings(self, silent: bool = False):
        ...
        # Startup options (moved to launcher)
        if hasattr(self, 'autostart_ollama'):
